...
export default function ModalNuevoProducto({ isOpen, onClose, onRefresh }: ModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            // 1. Crear el Producto (Sin el campo stock_inicial que rebota el backend)
            const resProducto = await fetch("http://localhost:3000/api/v1/productos", {
                method: 'POST',
            ...
            if (resProducto.ok) {
                const nuevoP = await resProducto.json();
                const productId = nuevoP.id_producto;

                // 2. Guardar Categoría (Si tu backend tiene tabla intermedia)
                await fetch("http://localhost:3000/api/v1/productos-categorias", {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ id_producto: productId, id_categoria: idCategoria })
                });

                // 3. Guardar Stock
                if (stockLocal > 0) {
                    await fetch("http://localhost:3000/api/v1/inventario", {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({
                ...
                await onRefresh(); // Actualiza la tabla principal
                onClose();
